...
<script src="../js/common.js" type="text/javascript" charset="utf-8"></script>
	<body>
		<div class="wrap-container clearfix">
				...
				<!-- form表单，将搜索按钮变为提交按按钮 -->
					<form class="layui-form" action="../lession/findLessionByTeacher.do" method="post">
					 ...
					 <c:if test="${ not empty lesTempList  }">
					...
					<table class="layui-table" lay-even lay-skin="nob">
						<colgroup>
							...
							<tbody>
							<c:forEach items="${lesTempList}" var="les" varStatus="status">
									<tr>
									...
									<td><a href="javaScript:;" class="layui-btn layui-btn-mini layui-btn-normal cla_edit-btn" data-id="${les.classId }"  data-text="班级学生管理" data-url="../user/getStudentsByClass.do">${les.className }</a></td>
									<td class="hidden-xs" >${les.professionName }</td>
									<td>
										<div class="layui-inline">
											<button class="layui-btn layui-btn-mini layui-btn-normal see_les_edit-btn" data-id="${les.lessionId}" data-url="../lession/findLessionByTeacherDetail.do"><i class="layui-icon">&#xe642;</i></button>
										</div>
									</td>
								</tr>
							</c:forEach>
...
<script type="text/javascript">
	...
	function firstPage(){
		...
		window.location.href="../lession/findLessionByTeacher.do?currentPage=1&keyWords="+name+"&pageSize="+pageSize;
	}
	function finalPage(){
		var name = document.getElementById("seacher").value;
		window.location.href="../lession/findLessionByTeacher.do?currentPage="+${totalPage }+"&keyWords="+name+"&pageSize="+pageSize;
	}
	function nextPage(){
		var name = document.getElementById("seacher").value;
		window.location.href="../lession/findLessionByTeacher.do?currentPage="+${currentPage + 1 }+"&keyWords="+name+"&pageSize="+pageSize;
	}
	...
	function goDis(){
		...
		window.location.href="../lession/findLessionByTeacher.do?&keyWords="+name+"&pageSize="+pageSize+"&currentPage="+currentPage;
	}
</script>		
</html>